...
App({
  globalData: {
    restaurantSets: [],
    openid: '', // 用于存储用户唯一标识
    isDataLoaded: false, // 标记数据是否已加载完成
    cloudEnabled: false, // 标记云开发是否可用
  },
  ...
  onLaunch() {
    ...
    if (!wx.cloud) {
      ...
      wx.cloud.init({
        env: 'eat-choose-cloud-6f6rreg2491cca3', // 请确保这是正确的环境ID
        traceUser: true,
      });
  ...
  async getOpenid() {
    try {
      ...
      else if (res && res.result && res.result.userInfo && res.result.userInfo.openId) {
      ...
      wx.showModal({
        title: '云同步初始化失败',
        content: '无法获取用户标识，请检查云函数是否正确部署。错误信息：' + e.message,
        showCancel: false
      });
      throw e;
    }
